...
public class KeyUtil {
     ...
     * @throws Exception 若加密過程中發生錯誤時拋出。
     */
    public static byte[] encryptWithPublicKey(PublicKey publicKey, byte[] bs) throws Exception {
        Cipher cipher = Cipher.getInstance("RSA");
        cipher.init(Cipher.ENCRYPT_MODE, publicKey);
        byte[] encryptedData = new byte[0];
        for (int i = 0; i < bs.length; i += CHUNK_SIZE) {
            byte[] chunk = new byte[Math.min(CHUNK_SIZE, bs.length - i)];
            System.arraycopy(bs, i, chunk, 0, chunk.length);
